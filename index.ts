...
type Transition = 
  | { type: "HitYourLeader" }
  | { type: "HitOpponentLeader" }
  | { type: "HitYourFollower"; id: number }
  | { type: "HitOpponentFollower"; id: number };
...
function followerIsAlive(follower: Follower): boolean {
    return follower.health > 0;
}

function getNAliveFollowers(followers: Follower[]): number {
    let count = 0;
...
function calculateSub(state: GameState, result: Result) {
...
function hitFollower(follower: Follower) {
...
function transitToNextState(state: GameState, result: Result, nextProb: Fraction, transition: Transition) {
...
function calculate(state: GameState): Result {
